...
firebaseAdmin.initializeApp(
  functions.config().firebase || {
    credential: firebaseAdmin.credential.cert(require("./serviceAccount.json")),
  }
);
...
const backend = firebaseAdmin.initializeApp(
  {
    credential: firebaseAdmin.credential.cert(
      require("./serviceAccount_backend.json")
    ),
  },
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    functions.logger.log(
      `Created new 2-week session cookie for user "${decodedClaims.email}" (ID: ${decodedClaims.user_id})`
    );
...
router.post("/logout", async (req, res) => {
  ...
  try {
    ...
    functions.logger.log(
      `Logged out user "${decodedClaims.email}" (ID: ${decodedClaims.user_id})`
    );
...
router.post("/validate/session", async (req, res) => {
  ...
  try {
    ...
    functions.logger.log(
      `Decoded session for user "${decodedClaims.email}" (ID: ${decodedClaims.user_id})`
    );
...
router.get("/messages", async (req, res) => {
  ...
  try {
    ...
    const snapshot = await firestore_backend
      .collection("messages")
      .limit(20)
      .get();
...
router.get("/publicmessages", async (req, res) => {
  ...
  if (req.query) {
    ...
    if (req.query["categories"]) {
      ...
      var categories = "illicit,untrue,misleading,accurate,scam,spam";
    }
    if (req.query["status"]) {
    ...
    if (req.query["report_count"]) {
      var report_count = req.query["report_count"];
    } else {
      var report_count = "";
    }
    if (req.query["report_date_start"]) {
      var report_date_start = req.query["report_date_start"];
      var date_filter_start =
        "firstReceivedUnixTimestamp:>" + report_date_start;
    } else {
  ...
  switch (status) {
    case "reviewed":
      filter = filter + " && isAssessed:=true";
      break;
    case "reviewing":
      filter = filter + " && isAssessed:=false";
      break;
    default:
      filter = filter + " ";
  }
  ...
  switch (report_count) {
    case "1":
      filter = filter + " && instanceCount:<=5";
      break;
    case "6":
      filter = filter + " && instanceCount:>5 && instanceCount:<=10";
      break;
    case "11":
      filter = filter + " && instanceCount:>10 && instanceCount:<=20";
      break;
    case "20":
      filter = filter + " && instanceCount:>20";
      break;
    default:
      filter = filter + " ";
  }
...
router.post("/export-notion", async (_, res) => {
  try {
    ...
    functions.logger.error("Error exporting Notion data to Firestore", err);
