...
const Member = ({ name, title, image }) => (
  <div className={`flex flex-col pl-${lpadding} gap-y-2 text-left`}>
    <img
      className="h-auto object-cover md:w-72 sm:w-48 w-36"
      src={image}
      alt={name}
    />
    <div>
...
const Check = () => {
  ...
  const updateSizes = () => {
    ...
    setFullWidth(
      imageSizeInPx * members.length + paddingInPx * (members.length + 1)
    );
  };
  ...
  return (
    ...
    >
      <motion.div
        className="block"
        style={{ maxHeight: `${dimensions.height}px`, y }}
      >
          ...
          <div className="bg-checkBG mt-24 sm:px-16 px-6 flex flex-col justify-center items-center">
            <div className="xl:max-w-[1280px] w-full flex flex-col justify-center">
              ...
              <div className="flex flex-row gap-x-6 pt-24 w-full justify-center mx-auto">
                ...
                >
                  <span className="text-checkWhite">
                    Join as a fact checker
                  </span>
                  <img src={arrowRight} />
                </a>
              </div>
